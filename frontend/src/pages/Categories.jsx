...
const Categories = () => {
  ...
  const handleSizeInputChange = (e) => {
    ...
    if (sizeType === 'Standard') {
      if (/^[a-zA-Z]*$/.test(val)) setSizesInput(val.toUpperCase());
    } else if (sizeType === 'Numeric') {
      if (/^[0-9]*$/.test(val)) setSizesInput(val);
    } else {
      setSizesInput(val);
    }
  };
  ...
  const handleRemoveChip = (val) => {
    setSizes(sizes.filter(item => item !== val));
  };
  ...
  const handleSave = () => {
    ...
    const url = `${API_URL}/category` + (editingId ? `/${editingId}` : '');
              ...
              <>
                ...
                <div className="flex gap-3">
                  <input 
                    ...
                    placeholder={sizeType === 'Standard' ? "Add Size (e.g. XL)" : "Add Size (e.g. 40)"} 
                  />
                  <button 
                    className="btn btn-secondary w-auto px-8"
  ...
  return (
    <div className="container animate-fade-in">
      <div className="flex justify-between items-center mb-10">
        <div>
           <h1 className="title mb-1">Collections</h1>
           <p className="text-secondary">Category Architecture</p>
        </div>
        <button 
      ...
      <div className="flex flex-col gap-4">
        {categories.map(cat => (
          <div key={cat.id} className="card group" onClick={() => openEditor(cat)} style={{cursor: 'pointer', padding: '24px', position: 'relative'}}>
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
              <div style={{flex: 1, minWidth: 0}}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{cat.name}</h3>
                  {cat.parent_id && <span style={{ padding: '2px 8px', borderRadius: '4px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '9px', fontWeight: '900', letterSpacing: '2px', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Sub-Category</span>}
                </div>
                <div style={{ fontSize: '10px', fontWeight: '900', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '2px' }}>
                   Dimensions: <span style={{ color: 'rgba(255,255,255,0.6)' }}>{cat.sizes?.length > 0 ? cat.sizes.join(', ') : 'Default'}</span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '12px', flexShrink: 0, marginLeft: '16px' }}>
                <button 
                  ...
                  style={{ 
                    ...
                    backgroundColor: 'rgba(255,59,59,0.15)', backdropFilter: 'blur(10px)', 
                    border: '1px solid rgba(255,59,59,0.3)', color: '#ff3b3b', 
                    display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', flexShrink: 0
                  }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(255,59,59,0.25)'; e.currentTarget.style.transform = 'scale(1.1)'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'rgba(255,59,59,0.15)'; e.currentTarget.style.transform = 'scale(1)'; }}
                >
                  <Trash2 size={16}/>
                </button>
                <div 
