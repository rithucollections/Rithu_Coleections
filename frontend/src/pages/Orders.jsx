...
const Orders = () => {
  ...
  return (
    <div className="container animate-fade-in">
      <div className="mb-10">
         <h1 className="title mb-1">Orders</h1>
         <p className="text-secondary">Track and fulfill customer requests</p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        {orders.map(order => (
          <div key={order.id} className="card p-0 overflow-hidden">
            <div className="p-6 flex justify-between items-center bg-white/5 border-b border-white/5">
              ...
              <div className={`${getBadgeClass(order.status)} px-3 py-1.5 rounded-xl border border-current bg-transparent`}>
                 {order.status}
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between items-center mb-8 px-2">
                ...
                <span className="text-2xl font-black text-gold">₹{order.total_price}</span>
              </div>
              
              <div className="grid-2">
                {order.status === 'Pending' && (
                  <button className="btn shadow-xl shadow-gold/10" onClick={() => updateStatus(order.id, 'Confirmed')}>
                    <CheckCircle size={18}/> Confirm Order
                  </button>
                )}
                {order.status === 'Confirmed' && (
                  <button className="btn shadow-xl shadow-success/10" onClick={() => updateStatus(order.id, 'Delivered')}>
                    <Truck size={18}/> Dispatch
                  </button>
                )}
                {order.status === 'Delivered' && (
                   <div className="flex items-center gap-2 text-success px-4 bg-success/5 rounded-xl border border-success/10 justify-center">
                      <CheckCircle size={16} /> <span className="text-[10px] font-black uppercase tracking-widest">Completed</span>
                   </div>
                )}
                <button className="btn btn-secondary" onClick={() => handleWhatsApp(order)}>
                  Notify via WhatsApp
                </button>
              </div>
            </div>
        ...
        {orders.length === 0 && (
          <div className="card text-center py-20 border-dashed">
             <div className="text-white/20 text-sm italic">No orders in the queue...</div>
          </div>
        )}
      </div>
    </div>
